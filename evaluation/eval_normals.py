...
def eval_normals(loader, folder):
    ...
    for i, sample in enumerate(loader):
        ...
        pred = 2. * \
            cv2.imread(filename).astype(float)[..., ::-1] / 255. - 1
        ...
        if pred.shape != label.shape:
            ...
            pred = cv2.resize(
                pred, label.shape[::-1], interpolation=cv2.INTER_CUBIC)
        ...
        deg_diff_tmp = np.rad2deg(
            np.arccos(np.clip(np.sum(pred * label, axis=2), a_min=-1, a_max=1)))
