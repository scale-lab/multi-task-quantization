...
def eval_sal(loader, folder, mask_thres=None):
    ...
    for i, sample in enumerate(loader):
        ...
        for j, thres in enumerate(mask_thres):
            ...
            eval_result['all_jaccards'][i,
                                        j] = evaluation.jaccard(gt, mask_eval)
            eval_result['prec'][i, j], eval_result['rec'][i,
                                                          j] = evaluation.precision_recall(gt, mask_eval)
...
def eval_sal_predictions(database, save_dir, overfit=False):
    ...
    eval_results = eval_sal(db, os.path.join(
        save_dir, 'sal'), mask_thres=np.linspace(0.2, 0.9, 15))
