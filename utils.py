...
def load_checkpoint(config, model, optimizer, lr_scheduler, loss_scaler, logger, backbone=False, quiet=False):
    ...
    #     if "attn_mask" in key or "relative_position" in key:
    #         to_delete_keys.append(key)

    # for key in to_delete_keys:
    #     print(f"Deleting key {key}")
    #     del checkpoint["model"][key]
    skip_decoder = config.MODEL.SKIP_DECODER_CKPT
...
def get_tokens_select_stats(NTs, weighted=False):
    ...
    for Nt, weight in zip(NTs, weights):
        ...
        for head in Nt:
            ...
            num_activated_tokens_per_layer.append(
                weight*torch.sum(head, dim=(1, 2))/head.shape[1])
    ...
    for Dt in DTs_pertask:  # stages
        for head in Dt:  # blocks
            for task in tasks:  # tasks
                num_activated_tokens_per_layer_per_task[task].append(
                    torch.sum(head[task], dim=(1, 2))/head[task].shape[1])
